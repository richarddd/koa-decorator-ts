...
const validateClass = (Type, type) => (...args) => utils_1.Decorate(args, (ctx, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (Array.isArray(err)) {
            const errors = err;
            const firstNestedChildError = (e, property = undefined) => {
                const targetProperty = ((property && property + '.') || '') + e.property;
                let target = e;
            ...
            throwValidationErrors(ctx, type, errors.map(e => {
                const childError = (e.children && e.children[0] && firstNestedChildError(e)) ||
                    undefined;
                const targetError = childError || e;
                return {
                    property: (childError && childError.property) || type,
                    message: (!e.value && `requires property \"${targetError.property}\"`) ||
